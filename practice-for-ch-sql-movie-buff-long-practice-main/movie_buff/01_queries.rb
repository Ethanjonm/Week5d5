...
def harrison_ford
  ...
  Movie.select(:id, :title)
    .joins(:actors)
    .joins(:castings) 
    .where('actors.name LIKE \'Harrison Ford\' AND castings.ord != 1 ').distinct
end
...
def biggest_cast
  ...
  # actors).
  Movie.select(:id, :title).joins(:actors).group('castings.movie_id').order('COUNT(castings.actor_id) DESC')
end
